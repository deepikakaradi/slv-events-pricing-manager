...
export default function QuoteBuilder() {
  ...
  const exportPDF = () => {
    ...
    finalY += 6;
    doc.setFont('Helvetica', 'normal');
    doc.text('Booking Advance (30%):', 130, finalY);
    doc.text(formatCurrency(calcResult.advance_payment).replace('₹', 'Rs. '), 175, finalY);

    finalY += 6;
    doc.text('Remaining Balance:', 130, finalY);
    doc.text(formatCurrency(calcResult.remaining_balance).replace('₹', 'Rs. '), 175, finalY);
    ...
    <div className="space-y-8 font-sans max-w-5xl">
      ...
      {step === 4 && calcResult && (
        <div className="space-y-8">
          {calcResult.custom_quote_required ? (
            <div className="backdrop-blur-xl bg-white/40 dark:bg-slate-900/40 shadow-2xl p-8 rounded-2xl border border-white/20 dark:border-slate-800/40 text-center space-y-6 max-w-xl mx-auto">
              <div className="w-16 h-16 rounded-full bg-amber-500/10 flex items-center justify-center text-amber-500 mx-auto border border-amber-500/20">
                <AlertTriangle className="h-8 w-8 animate-pulse" />
              </div>
              <div>
                <h3 className="font-outfit font-bold text-xl text-slate-800 dark:text-white">Custom Quotation Required</h3>
                <p className="text-sm text-slate-400 mt-2">
                  Standard package rates and predefined slabs do not apply for events hosting more than 500 guests.
                </p>
                <p className="text-xs text-luxury-500 font-medium mt-3 italic pl-4 pr-4">
                  "{calcResult.ai_recommendations?.summary}"
                </p>
              </div>
              <div className="pt-4 flex flex-col space-y-3">
                <div className="p-4 bg-slate-100/50 dark:bg-slate-950/60 rounded-xl text-xs text-slate-400 text-left space-y-2 border border-slate-200/50 dark:border-slate-800/40">
                  <div className="font-semibold text-slate-300">Next Steps:</div>
                  <ul className="list-disc pl-4 space-y-1">
                    <li>Contact standard vendor operations for customized catering quotes.</li>
                    <li>Submit venue and security guidelines to Admin for manual review.</li>
                    <li>Coordinate with the Sales Director for customized multi-tier margins.</li>
                  </ul>
                </div>
                <button
                  onClick={() => setStep(1)}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 glass-card p-8 rounded-2xl border border-white/20 dark:border-slate-800/40 shadow-xl space-y-6">
                  ...
                  <div className="space-y-2 border-b border-slate-200/50 dark:border-slate-800/40 pb-6">
                    ...
                    {calcResult.items.map((item, idx) => (
                      <div key={idx} className="flex justify-between text-xs font-mono text-slate-700 dark:text-slate-300">
                        ...
                        <span>{item.total === 0 ? 'Included' : formatCurrency(item.total)}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="space-y-4 font-mono text-sm border-t border-slate-200/50 dark:border-slate-800/40 pt-6">
                    ...
                    <div className="flex justify-between text-xs text-slate-400 border-t border-slate-150/40 dark:border-slate-800/40 pt-3">
                      <span>Booking Advance (30%):</span>
                      <span className="font-bold text-luxury-500">{formatCurrency(calcResult.advance_payment)}</span>
                    </div>
                    <div className="flex justify-between text-xs text-slate-400">
                      <span>Remaining Due (Before Event):</span>
                      <span className="font-bold text-slate-300">{formatCurrency(calcResult.remaining_balance)}</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="backdrop-blur-xl bg-white/40 dark:bg-slate-900/40 shadow-2xl p-6 rounded-2xl border border-white/20 dark:border-slate-800/40 shadow-sm space-y-4">
                    ...
                    {calcResult.ai_recommendations?.upsell ? (
                      <div className="space-y-3">
                        ...
                        <div className="p-2 bg-slate-100/50 dark:bg-slate-950/60 rounded-lg text-[9px] font-mono text-slate-400 flex justify-between">
